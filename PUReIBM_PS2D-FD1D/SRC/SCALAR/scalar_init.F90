    ...
    IF(iscal_restart.EQ.0)THEN
       DO isp=1,nspmx
          ...
          DO i=1,nx+1,1
             ...
             DO k=1,mz,1
                DO j=1,my,1
                   phir1(j,k) = one ! Initialize scalar field with inlet bulk temperature
                END DO
             END DO
...
!!$     ALLOCATE(phif(0:nx+2,my2,mz,nspmx))
!!$  else
!!$     ALLOCATE(phif(0:nx+1,my2,mz,nspmx))
!!$  end if
  ...
  ALLOCATE(heat_ratio(nspmx), heat_ratio_new(nspmx))
  ALLOCATE(heat_ratio_old(nspmx))


  ALLOCATE(um(nx+1), phim(nx+1,nspmx), countfl_plane(nx+1))
