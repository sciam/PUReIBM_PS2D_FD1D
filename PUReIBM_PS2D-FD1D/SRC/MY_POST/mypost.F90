...
!	public :: dissip_continuous, compute_sijsij
	...
	real(prcn) :: ufmean_mag, usmean1_mag, usmean2_mag, mix_mean_vel_mag, mix_mean_slip_mag, sijsij, sijsij_pure, tke_pure, source, dissipation, int_tke_pure, collision_timescale

	real(prcn), allocatable :: mix_mean_slip(:), mean_spec_vel_mag(:), &
		& mix_mean_vel(:), grant(:), tke_s_pure(:), acc_mean(:), acc_variance(:), vel_mean(:)

!	integer :: nvar, nvar1, nvar2, 
	integer :: line=1


	type :: slice_type_num_dens
		real(prcn) :: start, end
		real(prcn), allocatable :: density(:)
	end type slice_type_num_dens

	type(slice_type_num_dens), allocatable :: num_dens_slice_time(:)
...
contains
	...
	subroutine number_density_x
		implicit none

		type(slice_type_num_dens),allocatable :: slice(:)

		integer :: islice, ibody, iphs, pstart, pend, nslice
		integer :: nvar, nvar1, nvar2
		character*80 filename
		real(prcn) :: deltaslice, check_sum
		logical, save :: first_time = .true.

		if (.not. post_no_flow_mem_alloc) then
			nslice = int(lybyd)
			deltaslice = lybyd / nslice

			allocate(slice(nslice))

			if (first_time) then
				allocate(num_dens_slice_time(nslice))
			endif
		
			do islice=1, nslice
				slice(islice)%start = (islice-1) * deltaslice
				slice(islice)%end   = (islice)   * deltaslice
				allocate(slice(islice)%density(nphases))
				slice(islice)%density(:) = zero

				if (first_time) then
					num_dens_slice_time(islice)%start = (islice-1) * deltaslice
					num_dens_slice_time(islice)%end   = (islice)   * deltaslice
					allocate(num_dens_slice_time(islice)%density(nphases))
					num_dens_slice_time(islice)%density(:) = zero
				endif
			enddo
			...
			do iphs=1, nphases
				pstart = phase_array(iphs)%pstart
				pend   = phase_array(iphs)%pend
				do ibody = pstart, pend
					do islice=1, nslice
						if ( slice(islice)%start <= xc(ibody,1)/dbydx .and. xc(ibody,1)/dbydx <  slice(islice)%end) then
							slice(islice)%density(iphs) = slice(islice)%density(iphs) + one
							exit
						endif

						if (islice==nslice) then
							if ( xc(ibody,1)/dbydx >= slice(nslice)%end ) then
								slice(nslice)%density(iphs) = slice(nslice)%density(iphs) + one
							elseif ( xc(ibody,1)/dbydx < slice(1)%start ) then
								slice(1)%density(iphs) = slice(1)%density(iphs) + one
							else
								write (*,*) ibody,"th PARTICLE DOES NOT BELONG TO ANY SLICE"
								write (*,"(3d15.7)") xc(ibody,:)/dbydx
								stop
							endif
						endif
					enddo
			...
			check_sum = zero
			do islice=1, nslice
				slice(islice)%density(:) = slice(islice)%density(:)/nbody
				check_sum = check_sum + sum(slice(islice)%density(:))

				num_dens_slice_time(islice)%density(:) = num_dens_slice_time(islice)%density(:) + slice(islice)%density(:)
			enddo

!			write (*,"(1A,1d15.7)") "CHECK SUM = ", check_sum

			filename = TRIM(RUN_NAME)//"_num_dens.dat"
			open (unit=1, file=trim(filename), status="replace", action="write")
			write (1,*) "zone"

			do islice=1, nslice
				if (nphases == 1) then
					write (1,"(2d15.7)") (slice(islice)%start+slice(islice)%end)/2, slice(islice)%density(:)
				elseif (nphases == 2) then
					write (1,"(3d15.7)") (slice(islice)%start+slice(islice)%end)/2, slice(islice)%density(:)
				endif
			enddo
			close(1)
			...
			line = 1
			filename = "_num_dens.dat"
			call mis_average(nvar, nvar1, nvar2, filename, line, include_nmis)
		endif
	end subroutine number_density_x


	subroutine number_density_x_time
		implicit none
		integer :: partunit, io, count
		integer :: nvar, nvar1, nvar2
		real(prcn) :: t_base
		integer :: sample, islice, nslice
		character*80 filename
...
!			t_base = 50
			do
				call read_part_info(partunit,count,io)
				if (count==1) t_base = t
				if (io<0) exit

				if (t>=t_base) then

					call number_density_x
					sample = sample + 1
					t_base = t_base + 5

write (*,"(1a,d15.7,2i)") "TIME, COUNT = ", t, count, sample
				endif
			enddo

			nslice = size(num_dens_slice_time,1)

write (*,*) "NSLICe = ", nslice

			do islice=1, nslice
				num_dens_slice_time(islice)%density(:) = num_dens_slice_time(islice)%density(:)/sample
			enddo


			filename = TRIM(RUN_NAME)//"_num_dens_time.dat"
			open (unit=1, file=trim(filename), status="replace", action="write")
			write (1,*) "zone"

			do islice=1, nslice
				if (nphases == 1) then
					write (1,"(2d15.7)") (num_dens_slice_time(islice)%start+num_dens_slice_time(islice)%end)/2, num_dens_slice_time(islice)%density(:)
				elseif (nphases == 2) then
					write (1,"(3d15.7)") (num_dens_slice_time(islice)%start+num_dens_slice_time(islice)%end)/2, num_dens_slice_time(islice)%density(:)
				endif
			enddo
			close(1)
			...
			line = 1
			filename = "_num_dens_time.dat"
			call mis_average(nvar, nvar1, nvar2, filename, line, include_nmis)
		endif
	end subroutine number_density_x_time



	subroutine collision_time
		implicit none
		integer :: partunit, gofrunit, timeunit, io, count
		integer :: nvar, nvar1, nvar2, i, j
		real(prcn) :: max_overlap
		integer :: sample, islice, nslice
		character*80 filename1, filename2
		logical, allocatable :: contact(:,:), contact_old(:,:)
		integer(8), allocatable :: collision_count(:)
		integer(8) :: collision_total, collision_current
		real(prcn) :: coll_time, t_start, t_end


		if (.not. post_no_flow_mem_alloc) then
			if (I_AM_NODE_ZERO) write (*,*) "IN COLLISION_TIME..."
			allocate(contact(nbody,nbody), contact_old(nbody,nbody))
			contact = .false.
			contact_old = .false.

			if (allocated(rad_bin)) deallocate(rad_bin)
			if (allocated(gofr_avg)) deallocate(gofr_avg)
			allocate(rad_bin(nbins), gofr_avg(nbins))
			allocate(collision_count(nbody))
			collision_count = 0


			filename1 = TRIM(RUN_NAME)//"_gofr.dat"
			gofrunit = 100010
			open (unit=gofrunit, file=trim(filename1), status="replace", action="write")

			filename2 = TRIM(RUN_NAME)//"_collision_time.dat"
			timeunit = 100011
			open (unit=timeunit, file=trim(filename2), status="replace", action="write")
			write (timeunit,*) "zone"
			...
			do
				...
				if (t>=t_start) then
					...
					do i=1, nbody-1
						do j=i+1, nbody
							if (i/=j) then
								if (.not.contact_old(i,j) .and. contact(i,j)) then
									collision_current = collision_current+2
									!collision_count(i) = collision_count(i)+1
									!collision_count(j) = collision_count(j)+1
								endif
							endif
						enddo
					enddo
				...
				collision_total = collision_total + collision_current !sum(collision_count(:))/2
				coll_time = (t_end-t_start)/(dble(collision_total)/nbody)
				collision_timescale = coll_time

				if (mod(count,skip_num)==0) then
					if (I_AM_NODE_ZERO) write (*,*) "COUNT = ", count

					write (gofrunit,*) "zone"
					do i=1, nbins
							if (gofr_avg(i)>small_number) write (gofrunit,"(2d15.7)") rad_bin(i),gofr_avg(i)
					enddo
				endif
				if (collision_total>0) write (timeunit,"(5d15.7)") (t_end-t_start), (t_end-t_start)*mix_mean_slip_mag/char_length, dble(collision_current), dble(collision_total), coll_time*mix_mean_slip_mag/char_length
...
10				continue
	...
	subroutine post_part_stat
		...
		integer :: ibody, ibin, count, io, islice, partunit
		...
		real(prcn) :: uf_tmp, ufr_tmp,af_tmp
...
!				stop
			...
			do
				...
				elseif(io==0) then
					if (mod(count,skip_num)==1) then
						write (*,"(1a,1i,1a)") "IN THE ", count, "th ZONE"


!uf_tmp = sqrt(dot_product(ufmean,ufmean))
!ufr_tmp = sqrt(dot_product(frame_vel,frame_vel))
!af_tmp = sqrt(dot_product(frame_accln,frame_accln))

!write (*,"(2d15.7)") ufr_tmp, af_tmp
!read (*,*)
...
!						call find_clusters(xc, contact, nbody, lybyd, filename4, .true.)
						...
						write (10002,*) "zone"
						write (10002,"(4d15.7)") ((xc(ibody,:), radbdy(ibody)), ibody=1, nbody)
...
!if (ibody==25) write (*,"(3d15.7)") xc(ibody,:)
				...
				if (islice<1.or.islice>slice_n) then
					...
					if (islice<1) islice = 1
					if (islice>slice_n) islice = slice_n
...
!			write (2,"(1a,1i,1a,1i,1a)") "zone i= ", nbins, " j= ", slice_n, " F=point"
			...
			do islice=1, slice_n
				write (2,*) 'ZONE T = "', t/t_conv/(1-mean_volfrac), '",'
				if (slice_y(islice)%num>0) then
					do ibin=1, nbins
			...
			write (3,*) 'ZONE T = "', t/t_conv/(1-mean_volfrac), '",'

			do islice=1, slice_n
!				if (slice_y(islice)%num>0) then
					tmp1 = sqrt(dot_product(slice_y(islice)%usmean(:),slice_y(islice)%usmean(:)))
					tmp2 = ucharmod/(1-mean_volfrac)   !sqrt(dot_product(ufmean(:),ufmean(:)))
...
!			if (count>=100) then
				...
				if (ilu>0.and.ihu>0.and.ilu/=ihu) then
					write (4,*) 'ZONE T = "', t/t_conv/(1-mean_volfrac), '",'
				...
				write (5,"(4d15.7)") t/t_conv/(1-mean_volfrac), deltan, sqrt(grant), deltan/sqrt(grant)
!			endif
			close(3)
			close(4)
			close(5)
...
!						acc_part_rad(ibin)   = acc_part_rad(ibin)   / num_bin(ibin)
				...
				norm_factor = 3.d0*pi*vis*mix_mean_slip_mag*(1-mean_volfrac)*dia_phys
...
!						out_arr(i,j,k,ndim+1) = out_arr(i,j,k,ndim+1) / mean_energy
						...
						enddo
						out_arr(i,j,k,ndim+2) = tmp / 2 !umeanslip**2
					enddo
				enddo
			enddo
			...
			write(sunit,*)'ZONE T = "', t/t_conv/mean_volfrac, '",'
			...
			do k=1, mz
				do jj=1, 3
					...
					do i=1, mx1
						write(sunit,"(6d15.7)") i+frame_pos(1), j+frame_pos(2), k+frame_pos(3), out_arr(i,jj,k,5) / mean_energy !out_arr(i,jj,k,4) / mean_energy, out_arr(i,jj,k,5) / mean_energy, out_arr(i,jj,k,6) / umeanslip
					enddo
				enddo
			enddo
			...
			WRITE(sphrunit1,*)'ZONE T= "', t/t_conv/mean_volfrac, ' " '
			...
			do iphs=1, nphases
				...
				do m=part_start, part_end
					...
					write (sphrunit1,"(6d15.7)")  xc(m,:) + frame_pos(:), radbdy(m), tmp1 !, tmp2
  ...
  SUBROUTINE flow_snapshot3
	...
	if (I_AM_NODE_ZERO) then
      ...
      sunit     = 30
		...
		write(sunit,*)'ZONE T = "', t/t_conv/(1-mean_volfrac), '",'
		...
		WRITE(sphrunit1,*)'ZONE T= "', t/t_conv/(1-mean_volfrac), ' " '
	...
	subroutine write_drag
		...
		if (I_AM_NODE_ZERO) then
			if (.not. post_no_flow_mem_alloc) then
				...
				if (nphases==1) then
					call compute_ibm_drag(mean_volfrac, re, ibm_drag)

					write (1,"(4f10.4,4D15.7)") dbydx, lybyd, re, mean_volfrac, &
											& 	norm_drag, norm_drag_spec(1), ibm_drag, abs(norm_drag_spec(1)-ibm_drag) / ibm_drag
				else
					write (1,"(8f10.4,3D15.7)") dbydx, lybyd, re, mean_volfrac, &
				...
				write (1,"(4f10.4,3D15.7)") dbydx, lybyd, re, mean_volfrac, &
...
!						read (*,*)
			...
			do istep=1, nstep1
				x1 = phase_array(1)%volfrac !/mean_volfrac ! volfrac_rat
				x2 = phase_array(2)%volfrac !/mean_volfrac ! volfrac_rat

				var_u(1,istep) = var1(2,istep) / (one-mean_volfrac) !time

				mix_mean_slip_mag = var1(4,istep) * vis / (1-mean_volfrac) / char_length
			...
			do istep=1, nstep2
				var_d(1,istep) = var2(1,istep) / (one-mean_volfrac)
				...
				var_d(5,istep) = var2(5,istep) *3*pi*vis*phase_array(1)%dia*(one-mean_volfrac)*mix_mean_slip_mag * phase_array(1)%npart
				var_d(6,istep) = var2(6,istep) *3*pi*vis*phase_array(2)%dia*(one-mean_volfrac)*mix_mean_slip_mag * phase_array(2)%npart
			enddo
			...
			do istep=1, nstep1
				...
				if (match==.true.) then
					...
					var_out(14,kstep) = abs(usmean1_mag-ufmean_mag) * var_d(4,jstep) / (vis*(mix_mean_slip_mag/dia_phys)**2) &
									& / ((one-mean_volfrac)*doml(1)*doml(2)*doml(3))
			...
			do istep=1, kstep
				...
				var_out(28,istep) = -((var_out(21,istep) - (var_out(24,istep) - var_out(25,istep)))) / (1/rhos/phase_array(1)%volfrac+1/rhos/phase_array(2)%volfrac) * dia_phys**2/18/vis/mean_volfrac/(1-mean_volfrac)/mix_mean_slip_mag 
				var_out(29,istep) = -((var_out(21,istep) - (var_out(26,istep) - var_out(27,istep)))) / (1/rhos/phase_array(1)%volfrac+1/rhos/phase_array(2)%volfrac) * dia_phys**2/18/vis/mean_volfrac/(1-mean_volfrac)/mix_mean_slip_mag 
			...
			do istep=2, kstep
				write (1,"(1f12.4,28d15.7)") var_out(:,istep)
			enddo
			close(1)
				...
				do j=1, my
					do i=1, nx
						if (fluid_atijk(i,j,k)) then
							count = count+1
							do idim=1, ndim
								f_acc(idim, count) = ubcp(i,j,k,idim) - ufmean(idim)
							enddo
						endif
					enddo
				enddo
...
!				enddo
				...
				do i=1,nx
!					if (fluid_atijk(i,j,k)) then
						tmp = sqrt(dot_product(ubcp(i,j,k,:),ubcp(i,j,k,:)))
!						do idim=1,ndim
!							tmp = tmp + (ubcp(i,j,k,idim)-ufmean(idim)) * (ubcp(i,j,k,idim)-ufmean(idim)) / 2
!						enddo
						write (1,"(3i6,5d15.7)") i, j, k, tmp, ubcp(i,j,k,:)/ mixmeanslipmod, pbcp(i,j,k) / (half*mixmeanslipmod**2)
!					else
...
!					endif
	...
	subroutine volumetric_drag
		...
		if (.not.post_no_flow_mem_alloc) then
			...
			norm_force = 3*pi*dia_phys*vis*mix_mean_slip_mag*(1-mean_volfrac)
...
write (*,*) 3
				...
				vol_drag_average(:) = vol_drag_average(:) - mpg(:) * mean_volfrac * (dble(idelta)/dbydx*dia_phys)**3
...
!write (*,*) iii,jjj,kkk,2
				...
				write (1,"(7f12.4, 42D15.7)") dbydx, lybyd, mean_volfrac, re, rhos/rhof, coeff_rest, idelta*1.0/dbydx,			&
									& tr1(:,:), tr2(:,:), kr1, kr2, &
...
!		deallocate(out_arr)
  ...
  END SUBROUTINE flow_snapshot4
	...
	subroutine c_k
		...
		if (.not.post_no_flow_mem_alloc) then
			write (*,*) "C_K..."
			rhom = rhof*(1-mean_volfrac) + rhos*mean_volfrac
			...
			ef = rhof*(1-mean_volfrac)*tke_pure
			es = rhos* (mean_volfrac) *tke_s_pure(1)
			em = ef + es
			phi = rhos*mean_volfrac / (rhof*(1-mean_volfrac))
			...
			if (I_AM_NODE_ZERO) then
				...
				if (.not.imove==1) then
					if (nphases==1) then
						write (1,"(4f12.4,12D15.7)") dbydx, lybyd, mean_volfrac, re,								&
											&	phi, tp, tf1, tf2, stokes1, stokes2, ef, es, em, ck1, ck2, ck3
					endif
				else
					if (nphases==1) then
						write (1,"(7f12.4,18D15.7)") dbydx, lybyd, mean_volfrac, re, rhos/rhof, rhom/rhof, coeff_rest,	&
	...
	subroutine calc_scales
		...
		if (.not.post_no_flow_mem_alloc) then
			...
			stokes = rhos/rhof * ret/18
			write (*,"(1A,1D15.7)") "St(ReT) = ", stokes
			write (*,"(1A,1D15.7)") "St(Rem) = ", rhos/rhof * re/(1-mean_volfrac)/18


			tt1 = sqrt(3*grant(1)) / acc_mean(1)
			tt2 = sqrt(3*grant(1)) / acc_variance(1)
...
!			g = 8 * char_length**2 * nbody / (doml(1)*doml(2)*doml(3)) * sqrt(6. / (rhos*char_length**3)) * g_r
			g = 8 * char_length**2 * nbody / (doml(1)*doml(2)*doml(3)) * sqrt(pi) * g_r
			!tcoll = 2. / (g*sqrt(grant(1)))
			tcoll = collision_timescale

			write (*,"(1A,2d15.7)") "<A>, s_A = ", acc_mean(1), acc_variance(1)
			...
			write (1,"(6f12.4,10D15.7)") dbydx, lybyd, mean_volfrac, re, rhos, coeff_rest,	&
				&	ReT, grant(1)/mix_mean_slip_mag**2, rhos*mean_volfrac/rhof*3*grant(1)/mix_mean_slip_mag**2, acc_mean(1), acc_variance(1), &
				&	tt1*mix_mean_slip_mag/char_length, tt2*mix_mean_slip_mag/char_length, tcoll*mix_mean_slip_mag/char_length, tt1/tcoll, tt2/tcoll
			close (1)


			gamma_coll = 24./(char_length*sqrt(pi)) * (1.-coeff_rest) * rhos * mean_volfrac**2 * g_r * grant(1)**1.5
			write (*,"(1a,4d15.7)") "GAMMA_COLL/INT_TKE", gamma_coll / int_tke_pure
...
!			write (*,"(1A,1d15.7)") "GAMMA_(COLL+VIS)/DISIP_VIS = ", (gamma_coll+rhos*gamma_vis) / (int_tke + int_tke_fluc)
			...
			open (unit=1,file=trim(filename),status="replace",action="write")
			write (1,*) "zone"
			write (1,"(6f12.4,6D15.7)") dbydx, lybyd, mean_volfrac, re, rhos, coeff_rest,	&
...
!			g = 8 * char_length**2 * nbody / (doml(1)*doml(2)*doml(3)) * sqrt(6. / (rhos*char_length**3)) * g_r
			g = 8 * dia_phys**2 * nbody / (doml(1)*doml(2)*doml(3)) * sqrt(pi) * g_r
			tcoll = 2. / (g*sqrt(grant(1)))
			p8 = tt1 / tcoll
...
!			gamma_rat2 = p9/p10 !* p5**2 * p6/p8
			...
			pres_part = pres_part / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			visc_part = visc_part / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			int_tke   = int_tke   / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))

			pres_part_fluc = pres_part_fluc / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			visc_part_fluc = visc_part_fluc / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			int_tke_fluc   = int_tke_fluc   / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
...
!			write (*,"(1A,1d15.7)") "GAMMA_(COLL+VIS)/DISIP_VIS = ", (gamma_coll+rhos*gamma_vis) / (int_tke + int_tke_fluc)
			...
			write (1,"(6f12.4,15D15.7)") dbydx, lybyd, mean_volfrac, re, rhos, coeff_rest,	&
...
!			call mis_average(nvar, nvar1, nvar2, filename, line, include_nmis)
			...
			pres_part = pres_part / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			visc_part = visc_part / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			int_tke   = int_tke   / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))

			pres_part_fluc = pres_part_fluc / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			visc_part_fluc = visc_part_fluc / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))
			int_tke_fluc   = int_tke_fluc   / nbody !(doml(1)*doml(2)*doml(3)*(1-mean_volfrac))


			r_diss = (int_tke + int_tke_fluc) / (9*pi*vis*char_length*grant(1))

			kay = (1-mean_volfrac/2)/(1-mean_volfrac)**3
			gamma_rat = rhos/rhof * 4 * (1-coeff_rest) * mean_volfrac * kay * ReT / (9*sqrt(pi)*r_diss)
			...
			tcoll  = char_length/(24*mean_volfrac*kay)*sqrt(pi/grant(1))
			...
			write (1,"(6f12.4,3D15.7)") dbydx, lybyd, mean_volfrac, re, rhos, coeff_rest,	ReT, tfluid/tcoll, gamma_rat
			close (1)
		else
			nvar  = 9
			nvar1 = 6
...
!		h(:) = h(:) / nbody
		...
		do i=1, nbin
			write (1,"(3D15.7, 2F6.2)") (i-0.5) * dr, h_near(i)/total_n/dr, h_t(i)/total_t/dr, lybyd, mean_volfrac
		enddo
		close (1)
...
!				enddo
				...
				if (from_post) then
					...
					write (1,"(3D15.7)") t/dia_phys*umeanslip/(1-mean_volfrac), source/tmp1, dissipation/tmp1
				endif
				close (1)
...
!			open(unit=1, file=trim(filename), status="replace")
!			do ibody=1,nbody
!				write (1,"(1F6.2,1I4,1D15.7)") mean_volfrac, neighb(ibody), length(ibody)
!			enddo
!			close (1)
...
!				write (*,*) iconfig, i
	...
	subroutine compute_sijsij
		...
		logical :: neighb_insolid, filexist
...
!			sijsij_p   = sijsij_p   / count_fluid / (vis*(mix_mean_slip_mag/dia_phys)**2)
			...
			do iphs=1, nphases
				...
				do m=part_start, part_end
					do idim=1, ndim
						...
						pres_part(iphs)   = pres_part(iphs) + tmp1 * (pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0)
						visc_part(iphs)   = visc_part(iphs) + tmp1 * visc(m,idim)
						int_tke(iphs)     = int_tke(iphs)   + tmp1 * (visc(m,idim)+pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0)

						mix_pres_part   = mix_pres_part + tmp1 * (pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) !* phase_array(iphs)%volfrac
						mix_visc_part   = mix_visc_part + tmp1 * visc(m,idim) !* phase_array(iphs)%volfrac
						mix_int_tke     = mix_int_tke   + tmp1 * (visc(m,idim)+pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) !* phase_array(iphs)%volfrac
			...
			int_tke_pure = mix_int_tke / (doml(1)*doml(2)*doml(3)) !/(1-mean_volfrac)
...
mix_mean_vel_mag = (usmean1_mag*phase_array(1)%volfracg+usmean2_mag*phase_array(2)%volfracg)/mean_volfrac
...
write (*,"(3d15.7)") sqrt(dot_product(mesh_vel,mesh_vel))
				...
				do iphs=1, nphases
					...
					do m=part_start, part_end
						do idim=1, ndim
							...
							tmp2 = -(ufmean(idim)-phase_array(iphs)%mean_spec_vel(idim)) + tmp1

							pres_part_fluc(iphs) = pres_part_fluc(iphs) + (pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) * tmp1
							visc_part_fluc(iphs) = visc_part_fluc(iphs) + visc(m,idim) * tmp1
							int_tke_fluc(iphs)   = int_tke_fluc(iphs) + (visc(m,idim)+pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) * tmp1

							mix_pres_part_fluc = mix_pres_part_fluc + (pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) * tmp1
							mix_visc_part_fluc = mix_visc_part_fluc + visc(m,idim) * tmp1
							mix_int_tke_fluc   = mix_int_tke_fluc   - (visc(m,idim)+pres(m,idim)-mpg(idim)*pi*((two*radbdy(m)*dx)**3.d0)/6.d0) * tmp2
						enddo
					enddo
				enddo
			endif
			...
			do iphs=1, nphases
				mix_int_mom(:)     = mix_int_mom(:)     + int_mom(iphs,:)     * phase_array(iphs)%volfracg/mean_volfrac
				mix_total_force(:) = mix_total_force(:) + total_force(iphs,:) * phase_array(iphs)%volfracg/mean_volfrac
			enddo
			...
			enddo
			mix_pres_part = mix_pres_part / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)
			mix_visc_part = mix_visc_part / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)
			mix_int_tke   = mix_int_tke   / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)

			mix_pres_part_fluc = mix_pres_part_fluc / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)
			mix_visc_part_fluc = mix_visc_part_fluc / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)
			mix_int_tke_fluc   = mix_int_tke_fluc   / (doml(1)*doml(2)*doml(3)) &
										&	/ (vis*(mix_mean_slip_mag/dia_phys)**2)
...
!			mix_int_tke_fluc   = mix_int_tke_fluc   / mix_int_tke
			...
			if (I_AM_NODE_ZERO) then
				if (from_post) then
					...
					unitsijsij = 1
					open (unitsijsij, file=trim(filename), status="replace")
					write (unitsijsij,*) "zone"
				else
					filename = trim(run_name)//"_sijsij_runtime.dat"
					unitsijsij = 1

					if (irestart==0.and.first_pass) then
						open (unitsijsij, file=trim(filename), status="replace", action="write")
						write (unitsijsij,*) "zone"
					else
						inquire (file=trim(filename), exist=filexist)
						if (.not.filexist) then
							...
							open (unitsijsij, file=trim(filename), status="old", action="write", position="append")
						endif
					endif
				endif
				...
				if (from_post) then
					if (.not.imove==1) then
						if (nphases==1) then
							write (unitsijsij,"(4f10.4,9D15.7)") dbydx, lybyd, mean_volfrac, re, &
							& sijsij, sijsij_far, sijsij_far/sijsij, &
							& (int_tke(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
							& mix_int_tke, &
							& (int_tke_fluc(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke_fluc(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
							& mix_int_tke_fluc
						else
							write (unitsijsij,"(8f10.4,13D15.7)") dbydx, lybyd, mean_volfrac, &
							& (phase_array(iphs)%volfracg, iphs=1,nphases), &
						...
						if (nphases==1) then
							write (unitsijsij,"(6f10.4,9D15.7)") dbydx, lybyd, mean_volfrac, &
							& re, rhos, coeff_rest,	&
							& sijsij, sijsij_far, sijsij_far/sijsij,	&
							& (int_tke(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
							...
							write (unitsijsij,"(10f10.4,13D15.7)") dbydx, lybyd, mean_volfrac, &
							& (phase_array(iphs)%volfracg, iphs=1,nphases), &
							& (yalpha(iphs), iphs=1,nphases), re,			&
							& rhos, coeff_rest,								&
							& sijsij, sijsij_far, sijsij_far/sijsij,	&
					...
					if (.not.imove==1) then
						if (nphases==1) then
							write (unitsijsij,"(4f10.4,10D15.7)") dbydx, lybyd, mean_volfrac, re, &
							& t/t_conv/(1-maxvolfrac), sijsij, sijsij_far, sijsij_far/sijsij, &
							& (int_tke(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
							& mix_int_tke, &
							& (int_tke_fluc(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							...
							write (unitsijsij,"(8f10.4,14D15.7)") dbydx, lybyd, mean_volfrac, &
							& t/t_conv/(1-maxvolfrac), (phase_array(iphs)%volfracg, iphs=1,nphases), &
							& (yalpha(iphs),iphs=1,nphases), re,			&
							& sijsij, sijsij_far, sijsij_far/sijsij,		&
							& (int_tke(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
						...
						if (nphases==1) then
							write (unitsijsij,"(6f10.4,10D15.7)") dbydx, lybyd, mean_volfrac, &
							& re, rhos, coeff_rest,	&
							& t/t_conv/(1-maxvolfrac), sijsij, sijsij_far, sijsij_far/sijsij,	&
							& (int_tke(iphs) / (doml(1)*doml(2)*doml(3)), iphs=1,nphases),&
							& (int_tke(iphs) / phase_array(iphs)%npart, iphs=1,nphases),&
							...
							write (unitsijsij,"(10f10.4,14D15.7)") dbydx, lybyd, mean_volfrac, &
							& (phase_array(iphs)%volfracg, iphs=1,nphases), &
							& (yalpha(iphs), iphs=1,nphases), re,			&
							& rhos, coeff_rest,								&
							& t/t_conv/(1-maxvolfrac), sijsij, sijsij_far, sijsij_far/sijsij,	&
				...
				if (.not.imove==1) then
					...
					open (unit=1, file=trim(filename), status="replace")
					if (nphases==1) then
						write (1,"(4f12.4,2D15.7)") dbydx, lybyd, mean_volfrac, re,	&
						& (int_mom_mag(iphs), iphs=1, nphases), (total_force_mag(iphs), iphs=1, nphases)
					else
						write (1,"(8f12.4,6D15.7)") dbydx, lybyd, mean_volfrac, &
						& (phase_array(iphs)%volfracg, iphs=1,nphases),	&
						& (yalpha(iphs), iphs=1,nphases), re,											&
						& (int_mom_mag(iphs), iphs=1, nphases), (total_force_mag(iphs), iphs=1, nphases),&
...
!		fluc   = 0d0
	...
	!		fluc   = fluc   / (dot_product(ufmean(:)-usmean(:),ufmean(:)-usmean(:))/2)
			...
			pres_part = pres_part / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac)) / (vis*(mix_mean_slip_mag/dia_phys)**2)
			visc_part = visc_part / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac)) / (vis*(mix_mean_slip_mag/dia_phys)**2)

			int_tke_pure = int_tke   / (doml(1)*doml(2)*doml(3))
			int_tke      = int_tke   / (doml(1)*doml(2)*doml(3)) / (vis*(mix_mean_slip_mag/dia_phys)**2) !/ (1-mean_volfrac)

			pres_part_fluc = pres_part_fluc / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac)) / (vis*(mix_mean_slip_mag/dia_phys)**2)
			visc_part_fluc = visc_part_fluc / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac)) / (vis*(mix_mean_slip_mag/dia_phys)**2)
			int_tke_fluc   = int_tke_fluc   / (doml(1)*doml(2)*doml(3)*(1-mean_volfrac)) / (vis*(mix_mean_slip_mag/dia_phys)**2)
			...
			if (I_AM_NODE_ZERO) then
				...
				if (.not.imove==1) then
					if (nphases==1) then
						write (1,"(4f12.4,6D15.7)") dbydx, lybyd, mean_volfrac, re,			&
											& sijsij, sijsij_far, sqrt(sijsij)/mean_volfrac,		&
											& int_tke, int_tke/nbody, int_tke_fluc
					else
						write (1,"(8f12.4,11D15.7)") dbydx, lybyd, mean_volfrac, (phase_array(iphs)%volfracg, iphs=1,nphases), 	&
											& (yalpha(iphs),iphs=1,nphases), re,										&
											& sijsij, sijsij_far, sijsij_far/sijsij,				&
					...
					if (nphases==1) then
						write (1,"(6f12.4,6D15.7)") dbydx, lybyd, mean_volfrac, re, rhos/rhof, coeff_rest,	&
											& sijsij, sijsij_far, sijsij/sijsij_far,			&
											& int_tke, int_tke/nbody, int_tke_fluc
					else
						write (1,"(10f12.4,11D15.7)") dbydx, lybyd, mean_volfrac, (phase_array(iphs)%volfracg, iphs=1,nphases), &
											& (yalpha(iphs), iphs=1,nphases), re,										&
											& rhos, coeff_rest,														&
											& sijsij, sijsij_far, sijsij_far/sijsij,				&
											& (int_tke(iphs), iphs=1,nphases), mix_int_tke, 								&
...
!								& (int_mom_mag(iphs), iphs=1, nphases), (total_force_mag(iphs), iphs=1, nphases)
!					else
!						write (1,"(8f12.4,6D15.7)") dbydx, lybyd, mean_volfrac, (phase_array(iphs)%volfracg, iphs=1,nphases),	&
!								& (yalpha(iphs), iphs=1,nphases), re,											&
!								& (int_mom_mag(iphs), iphs=1, nphases), (total_force_mag(iphs), iphs=1, nphases), 		&
!								& mix_int_mom_mag, mix_total_force_mag
!					endif
...
!			enddo
			...
			if(I_AM_NODE_ZERO)then
				...
				if (from_post) then
					if (.not.imove==1) then
						if (nphases==1) then
							write (1,"(4F10.2,21D15.7)") dbydx, lybyd, mean_volfrac, re,	&
							& t/t_conv/mean_volfrac, tke, tke_far/tke, &
							& ((uiuj_f(i,j),i=1,ndim),j=1,ndim), &
							& ((bij_f(i,j),i=1,ndim),j=1,ndim)
						else
							write (1,"(6f10.2,21D15.7)") mean_volfrac, &
							& (phase_array(iphs)%volfracg, iphs=1,nphases), &
							& (yalpha(iphs), iphs=1,nphases), re,	&
							& t/t_conv/mean_volfrac, tke, tke_far/tke, &
							& ((uiuj_f(i,j),i=1,ndim),j=1,ndim), &
						...
						if (nphases==1) then
							write (1,"(6f10.2,31D15.7)") dbydx, lybyd, mean_volfrac, &
							& re, rhos, coeff_rest, &
							& t/t_conv/mean_volfrac, tke,tke_far/tke, &
							& ((uiuj_f(i,j),i=1,ndim),j=1,ndim), &
							& ((bij_f(i,j),i=1,ndim),j=1,ndim), &
							...
							write (1,"(8f10.2,42D15.7)") mean_volfrac,(phase_array(iphs)%volfracg, iphs=1,nphases),(yalpha(iphs), iphs=1,nphases), &
							& re, rhos, coeff_rest, &
							& t/t_conv/mean_volfrac, tke,tke_far/tke, &
							& ((uiuj_f(i,j),i=1,ndim),j=1,ndim), &
							& ((bij_f(i,j),i=1,ndim),j=1,ndim), &
					...
					write (1,"(11d15.7)") t/dia_phys*umeanslip/(1-mean_volfrac), tke, xi, eta1, bij_f(1,1), bij_f(2,2), bij_f(3,3), &
				...
				if (imove==1) then
					...
					open (unit=1,file=trim(filename),status="replace")
					write (1,"(4f12.4,4D15.7)") mean_volfrac, re, rhos, coeff_rest, tke, tke_mix_s, (tke*(1-mean_volfrac)*rhof + tke_mix_s*mean_volfrac*rhos), (tke*(1-mean_volfrac)*rhof + tke_s*mean_volfrac*rhos) / ((1-mean_volfrac)*rhof + mean_volfrac*rhos)
					close (1)
				endif
#endif
			endif
		...
		do imis=1, nmis
			...
			do
				...
				elseif (io1==0.and.io2==0) then
					istep = istep+1
					write (3,"(4f12.4,20D15.7)") var1(1:4), var2(1), var1(5:23)
...
!    PRINT*,'CONFIDENCE INTERVAL FOR ', TRIM(entry),' CONFIDENCE AND ',nsamples, ' DEGREES OF FREEDOM IS ', confis 
	...
	!		if (small_max_overlap>max_overlap) write (*,"(1a,1d15.7)") "max overlap = ", small_max_overlap
			...
			norm_force = 3.d0*pi*vis*mix_mean_slip_mag*(1-mean_volfrac)*dia_phys
			do ibody=1, nbody
...
!			line = nbins
	...
	contains
		...
		subroutine compute_force
			...
			real(prcn) :: total_force(ndim), total_force_mag, norm_force, tmp1, cluster_force, equi_d, equi_cluster_force, uni_force
			character*50 filename1
...
!				ibin = cluster(i)%num
			...
			do ibin=1, max_num
				...
				call compute_ibm_drag(mean_volfrac, re*equi_part_dia1(ibin)/dia_phys, equi_part_force1(ibin))
				call compute_ibm_drag(mean_volfrac, re*equi_part_dia2(ibin)/dia_phys, equi_part_force2(ibin))
			enddo

			norm_force = 3.d0*pi*dia_phys*vis*(1-mean_volfrac)*mix_mean_slip_mag
...
!					write (1,"(1f10.2, 4d15.7)") ibin, force_bin_mag(ibin)/force_bin(ibin)/norm_force, force_bin_mag(ibin)/norm_force, force_bin_mag(ibin)/total_force_mag, equi_part_force(ibin)
			...
			do i=1, ncluster
				cluster_force = sqrt(dot_product(cluster(i)%force(:),cluster(i)%force(:)))
				equi_d = dia_phys * (cluster(i)%num)**(1./3.)
				call compute_ibm_drag(mean_volfrac, re*equi_d/dia_phys, equi_cluster_force)
				call compute_ibm_drag(mean_volfrac, re, uni_force)

				write (1,"(2i,3d15.7)") i, cluster(i)%num, cluster_force/norm_force, cluster_force/norm_force/uni_force, equi_cluster_force
			enddo
			close(1)
		...
		subroutine compute_force_distribution_per_particle
			...
			real(prcn) :: norm_force, max_force, max_pres, max_visc, delta_force, delta_pres, delta_visc, max_part, delta_part
			...
			max_pres = maxval(pres_part_mag(:))
			delta_pres = max_pres/bins

			max_visc = maxval(visc_part_mag(:))
			delta_visc = max_visc/bins
			...
			do ibody=1, nbody
				ibin = int(pres_part_mag(ibody)/delta_pres)+1
				if (ibin>bins) ibin = bins
				pres_bin(ibin) = pres_bin(ibin)+1

				ibin = int(visc_part_mag(ibody)/delta_visc)+1
				if (ibin>bins) ibin = bins
			...
			write (1,*) "zone"
			do ibin=1, bins
				write (1,"(6d15.7)") (ibin-.5)/bins * max_pres, (ibin-.5)/bins * max_visc, (ibin-.5)/bins * max_force, pres_bin(ibin)/delta_pres, visc_bin(ibin)/delta_visc, total_bin(ibin)/delta_force
			enddo
			close(1)
...
!			max_part = maxval(cluster(:)%num)
			...
			if (ncluster>0) then
				do ibody=1, nbody
					...
					jbin = int(pres_part_mag(ibody)/delta_pres)+1
					if (jbin>bins) jbin = bins
					pres_bin_cluster(ibin,jbin) = pres_bin_cluster(ibin,jbin)+1

					jbin = int(visc_part_mag(ibody)/delta_visc)+1
					if (jbin>bins) jbin = bins
				...
				do ibin=1, cluster_bins
					do jbin=1, bins
						write (1,"(6d15.7)") (jbin-.5)/bins * max_pres, (jbin-.5)/bins * max_visc,(jbin-.5)/bins * max_force, pres_bin_cluster(ibin,jbin)/delta_pres, visc_bin_cluster(ibin,jbin)/delta_visc, total_bin_cluster(ibin,jbin)/delta_force
					enddo
				enddo
				close(1)
...
!		logical, intent(in) :: first_pass
		...
		if (first_pass) then
			open (unit=12, file=trim(filename1), status="replace")
		else
			open (unit=12, file=trim(filename1), status="old", position="append")
		endif

		write (12,*) "zone"
		write (12,"(4d15.7,1i)") ((xc(i,:), rad(i), color1(i)), i=1, nbody)
		close (12)
...
!		logical, intent(in) :: first_pass
	...
	SUBROUTINE calculate_gofr_energy(n, xc, vel, contact, my, mbox, nrbins, rescaling, gofr, e_par, e_per, radbin, overlap)
		IMPLICIT NONE

		Integer, Intent(in) :: mbox,my,n, nrbins
		Real(prcn), Intent(in) ::  xc(n,ndim), vel(n,ndim)
		logical, intent(inout) :: contact(n,n)
		real(prcn), intent(inout) :: overlap
		real(prcn), Intent(out) ::  gofr(nrbins), e_par(nrbins), e_per(nrbins), radbin(nrbins)

		Logical, INTENT(in) ::  rescaling

		Integer :: i,idim,j,ibin, dimi, dimj, count

		REAL(prcn) ::  L(ndim), volbin, rmax, rmin, dr, tempr(ndim), decimal, r, e_parallel, e_perpendicular, vel_parallel(ndim), vel_perpendicular(ndim), vel_relative(ndim)
...
!    enddo
		...
		do i = 1, n-1
			do j = i+1, n
				...
				tempr = tempr/r
				vel_relative(:) = vel(i,:) - vel(j,:)
				vel_parallel(:) = dot_product(vel_relative(:), tempr(:)) * tempr(:)
				vel_perpendicular(:) = vel_relative(:)-vel_parallel(:)

				e_parallel = dot_product(vel_parallel,vel_parallel)
				e_perpendicular = dot_product(vel_perpendicular,vel_perpendicular) / 2

				e_par(ibin) = e_par(ibin) + e_parallel
				e_per(ibin) = e_per(ibin) + e_perpendicular
...
1001			continue
		...
		do ibin=1,nrbins
			if (gofr(ibin)>small_number) then
				e_par(ibin) = e_par(ibin)/gofr(ibin)
				e_per(ibin) = e_per(ibin)/gofr(ibin)
			endif
	...
	end subroutine calculate_gofr_energy
