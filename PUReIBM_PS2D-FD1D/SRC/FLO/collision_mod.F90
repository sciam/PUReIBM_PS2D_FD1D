...
!!$    end do
  ...
  SUBROUTINE compute_vol_frac_cgrid
    ...
    jm = cy
    km = cz
    PRINT*,'In compue vol_fracr, interp_scheme = ',&
         & interp_scheme

    DO m=1,nbody          
       xl(1)=(xc(m,1)+foffset)*dx
       xl(2)=(xc(m,2))*dy
       xl(3)=(xc(m,3))*dz
       pc(:) = PIJK(m,:)
       !WRITE(*,*)'PC = ',xl
       ...
       do k = 1, onew
          do j = 1, onew
             do i = 1, onew
                ...
                if(ii.lt.1.and.intx_per) ii = cx+ii-1
                if(ii.gt.cx.and.intx_per) ii = ii-cx +1
...
!!$       TEST_PART = PARTICLES/2
		  TEST_PART = 1
       DO LL = 1, PARTICLES 
          DES_POS_OLD(LL,1) = (XC(LL,1)+foffset-one)*dx
...
!!$      END IF
    ...
    SUBROUTINE IBMUPDATE
      ...
      INTEGER M
      DO M=1,NBODY
         XC(M,1) = DES_POS_NEW(M,1)/dx + one - foffset
...
!!$         end do
         ...
         IF(TEST_YMAXVAL) THEN
            ...
            IF((MOD(IFAC,1000).EQ.0)) THEN
               PRINT*,'MAX - MIN = ', MAXVAL(DES_POS_NEW(1:PARTICLES,2))-MINVAL(DES_POS_NEW(1:PARTICLES,2))
